...
impl Cif {
    ...
    pub fn new_with_abi<I>(args: I, result: Option<Type>, abi: FfiAbi) -> Self
    where
        I: IntoIterator<Item = Type>,
    {
        ...
        unsafe { low::prep_cif(&mut cif, abi, nargs, result.as_ffi_ptr(), args.as_ffi_ptr()) }
            .expect("low::prep_cif");
    ...
    /// Takes ownership of the argument and result [`Type`]s, because
    /// the resulting [`Cif`] retains references to them.
    pub fn new_variadic_with_abi<I>(
        args: I,
        fixed_args: usize,
        result: Option<Type>,
        abi: FfiAbi,
