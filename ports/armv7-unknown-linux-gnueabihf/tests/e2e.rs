...
const FFI_DEFAULT_ABI: c_uint = 2; // FFI_VFP for arm-linux-gnueabihf
...
fn reports_libffi_version() {
    unsafe {
        assert_eq!(
            libffi_c2rust_armv7_gnueabihf::src::types::ffi_get_version_number(),
            30502
        );
        assert_eq!(core::mem::size_of::<FfiCif>(), 48);
    }
}
...
// point aggregate has special VFP calling-convention behavior on ARM hard-float.
#[derive(Clone, Copy)]
#[repr(C)]
struct ThreeFloats {
    f1: f32,
...
// Rust's armv7 `extern "C"` lowering does not currently return this HFA in
// VFP registers in debug builds, so use a tiny target-ABI assembly callee.
core::arch::global_asm!(
    ".syntax unified",
    ".fpu vfpv3-d16",
    ".text",
    ".global test_hfa_3f",
    ".type test_hfa_3f, %function",
    "test_hfa_3f:",
    "vmov.f32 s3, #1.0",
    "vadd.f32 s2, s2, s3",
    "bx lr",
    ".size test_hfa_3f, .-test_hfa_3f",
);
...
#[test]
#[ignore = "known translated ARM_TYPE_VFP_N return-path limitation; see PORTING.md"]
fn struct_by_value_3f_test() {
    unsafe {
        ...
        let mut arg_types = [struct_type];
        let mut cif = prep_cif(struct_type, &mut arg_types);
        assert_eq!(cif.flags, 2, "HFA must use ARM_TYPE_VFP_N");
        assert_ne!(cif.vfp_used, 0, "HFA argument must occupy VFP registers");
        ...
        call(
            &mut cif,
            test_hfa_3f as *const (),
            result.as_mut_ptr().cast(),
            &mut args,
        );
        let result = result.assume_init();
        assert!(
            (result.f3 - 556.5).abs() < f32::EPSILON,
            "unexpected HFA result: ({}, {}, {})",
            result.f1,
            result.f2,
            result.f3
        );
