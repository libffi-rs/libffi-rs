...
// C2Rust cannot lower Clang's __builtin___clear_cache. On GNU AArch64,
// libgcc provides this target cache-maintenance helper.
unsafe fn ffi_clear_cache(begin: *mut ::core::ffi::c_void, end: *mut ::core::ffi::c_void) {
...
const CALLEE_STACK_HEADROOM: usize = 1024 * 1024;
    ...
    match candidate {
        ...
        FFI_TYPE_STRUCT => {
            ...
            match candidate {
                ...
                FFI_TYPE_LONGDOUBLE => {
                    ele_count = size.wrapping_div(::core::mem::size_of::<crate::F128>() as size_t);
                    if size
                        != ele_count.wrapping_mul(::core::mem::size_of::<crate::F128>() as size_t)
                    {
                        return 0 as ::core::ffi::c_int;
                    }
                }
...
unsafe extern "C" fn ffi_call_int(
    ...
    let mut alloca_allocations: Vec<Vec<Aligned16>> = Vec::new();
    ...
    let allocation_bytes = CALLEE_STACK_HEADROOM
    ...
    alloca_allocations.push(::std::vec::from_elem(
        Aligned16([0; 16]),
        allocation_bytes.wrapping_add(15) / 16,
    ));
    context = alloca_allocations
        .last_mut()
        .unwrap()
        .as_mut_ptr()
        .add(CALLEE_STACK_HEADROOM / 16)
        .cast::<call_context>();
    ...
    while i < nargs {
        ...
        match t {
            ...
            | FFI_TYPE_POINTER => {
                current_block_57 = 7197522801683002542;
            }
            FFI_TYPE_UINT128 | FFI_TYPE_SINT128 => {
