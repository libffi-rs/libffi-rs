...
fn reports_libffi_version() {
    unsafe {
        assert_eq!(
            libffi_c2rust_aarch64_linux_gnu::src::types::ffi_get_version_number(),
            30502
        );
        assert_eq!(core::mem::size_of::<FfiCif>(), 32);
        assert_eq!(ffi_type_longdouble.size, 16);
        assert_eq!(ffi_type_longdouble.alignment, 16);
        assert_eq!(ffi_type_longdouble.kind, 4);
        assert_eq!(ffi_type_complex_float.size, 8);
        assert_eq!(ffi_type_complex_float.alignment, 4);
        assert_eq!(ffi_type_complex_double.size, 16);
        assert_eq!(ffi_type_complex_double.alignment, 8);
        assert_eq!(ffi_type_complex_longdouble.size, 32);
        assert_eq!(ffi_type_complex_longdouble.alignment, 16);
    }
}
...
fn closure_test() {
    unsafe {
        ...
        let mut cif = prep_cif(sint32, &mut arg_types);
        ...
        let function: unsafe extern "C" fn(i32, i32) -> i32 = core::mem::transmute(code);
        assert_eq!(function(10, 20), 37);
...
// point aggregate has special calling-convention behavior on the AArch64 PCS.
#[derive(Clone, Copy)]
#[repr(C)]
struct ThreeFloats {
    f1: f32,
