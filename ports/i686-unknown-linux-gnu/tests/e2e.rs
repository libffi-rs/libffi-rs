...
fn reports_libffi_version() {
    unsafe {
        assert_eq!(
            libffi_c2rust_i686_gnu::src::types::ffi_get_version_number(),
            30502
        );
    }
}
...
#[test]
fn target_layout_and_long_double_descriptors_match_i686_gnu() {
    unsafe {
        ...
        let complex = &raw const ffi_type_complex_longdouble;
        assert_eq!((*complex).size, 24);
        assert_eq!((*complex).alignment, 4);
    }
}

unsafe extern "C" fn add_bias_closure(
    _cif: *mut FfiCif,
    result: *mut c_void,
    args: *mut *mut c_void,
    user_data: *mut c_void,
) {
    let value = *(*args).cast::<i32>();
    let bias = *user_data.cast::<i32>();
    *result.cast::<i32>() = value + bias;
}

#[test]
fn closure_round_trip() {
    unsafe {
        let mut arg_types = [core::ptr::addr_of_mut!(ffi_type_sint32)];
        ...
        assert_eq!(
            ffi_prep_closure_loc(
                closure,
                &mut cif,
                Some(add_bias_closure),
                value_ptr(&mut bias),
                code,
            ),
            FFI_OK
        );
        let function = core::mem::transmute::<*mut c_void, unsafe extern "C" fn(i32) -> i32>(code);
        assert_eq!(function(5), 42);
        assert_eq!(function(-40), -3);
...
// Port of testsuite/libffi.call/struct_by_value_3f.c. A homogeneous floating-
// point aggregate also exercises small aggregate handling on i686.
#[derive(Clone, Copy)]
#[repr(C)]
struct ThreeFloats {
    f1: f32,
...
unsafe extern "C" fn struct_by_value_3f(mut value: ThreeFloats) -> ThreeFloats {
    value.f3 += 1.0;
    value
}

#[test]
fn struct_by_value_3f_test() {
    unsafe {
        let float_type = core::ptr::addr_of_mut!(ffi_type_float);
        let mut elements = [float_type, float_type, float_type, core::ptr::null_mut()];
        let mut struct_type = FfiType {
            size: 0,
            alignment: 0,
            kind: FFI_TYPE_STRUCT,
        ...
        let mut arg_types = [struct_type];
        let mut cif = prep_cif(struct_type, &mut arg_types);
        let mut value = ThreeFloats {
            f1: 1.0,
            f2: 2.0,
            f3: 555.5,
        };
        let mut result = core::mem::MaybeUninit::<ThreeFloats>::uninit();
        let mut args = [value_ptr(&mut value)];

        call(
            &mut cif,
            struct_by_value_3f as *const (),
            result.as_mut_ptr().cast(),
            &mut args,
        );
        let result = result.assume_init();
        assert!((result.f3 - 556.5).abs() < f32::EPSILON);
        assert!((value.f3 - 555.5).abs() < f32::EPSILON);
    }
}

// Proves the call shim gives the callee the real native stack rather than the
// heap Vec emitted by C2Rust for C alloca().
#[inline(never)]
unsafe extern "C" fn large_stack_callee(value: c_uint) -> c_uint {
    let mut scratch = [0_u8; 128 * 1024];
    core::ptr::write_volatile(scratch.as_mut_ptr(), 0x5a);
    core::ptr::write_volatile(scratch.as_mut_ptr().add(scratch.len() - 1), 0xa5);
    core::hint::black_box(&mut scratch);
    value + scratch[0] as c_uint + scratch[scratch.len() - 1] as c_uint
}

#[test]
fn ffi_call_callee_uses_native_stack() {
    unsafe {
        let uint32 = core::ptr::addr_of_mut!(ffi_type_uint32);
        let mut arg_types = [uint32];
        let mut cif = prep_cif(uint32, &mut arg_types);
        let mut value = 7_u32;
        let mut result = 0_u32;
        let mut args = [value_ptr(&mut value)];
        call(
            &mut cif,
            large_stack_callee as *const (),
            value_ptr(&mut result),
            &mut args,
        );
        assert_eq!(result, 7 + 0x5a + 0xa5);
    }
}
