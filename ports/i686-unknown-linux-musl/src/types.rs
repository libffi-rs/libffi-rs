...
// i686 SysV uses the 80-bit x87 format in a 12-byte C `long double`
// object with 4-byte alignment. Rust has no matching scalar type, so keep
// this ABI descriptor explicit instead of using C2Rust's incorrect f128.
#[no_mangle]
...
// C2Rust 0.22.1 does not lower C `_Complex` declarations. These are the
// exact configured i686 layouts: two adjacent components, aligned like one.
static mut ffi_elements_complex_float: [*mut ffi_type; 2] =
    [&raw mut ffi_type_float, ::core::ptr::null_mut()];
static mut ffi_elements_complex_double: [*mut ffi_type; 2] =
    [&raw mut ffi_type_double, ::core::ptr::null_mut()];
static mut ffi_elements_complex_longdouble: [*mut ffi_type; 2] =
    [&raw mut ffi_type_longdouble, ::core::ptr::null_mut()];
