...
pub struct max_align_t {
    ...
    pub __clang_max_align_nonce2: crate::F128,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn unmarshal_float(
    ...
    let fresh3 = (*cb).used_float;
    (*cb).used_float = (*cb).used_float + 1;
    value.i = (*(*cb).aregs).fa[fresh3 as usize].i as uint32_t;
...
unsafe extern "C" fn marshal_atom(
    ...
    match type_0 {
        ...
        FFI_TYPE_DOUBLE => {
            let fresh5 = (*cb).used_float;
            (*cb).used_float = (*cb).used_float + 1;
            (*(*cb).aregs).fa[fresh5 as usize].d = *(data as *mut ::core::ffi::c_double);
            return;
        }
        _ => {}
    }
    if (*cb).used_integer == NARGREG {
        let fresh6 = (*cb).used_stack;
        (*cb).used_stack = (*cb).used_stack.offset(1);
        *fresh6 = value;
    } else {
        let fresh7 = (*cb).used_integer;
        (*cb).used_integer = (*cb).used_integer + 1;
        (*(*cb).aregs).a[fresh7 as usize] = value;
    };
}
unsafe extern "C" fn unmarshal_atom(
    ...
    match type_0 {
        ...
        FFI_TYPE_DOUBLE => {
            let fresh0 = (*cb).used_float;
            (*cb).used_float = (*cb).used_float + 1;
            *(data as *mut ::core::ffi::c_double) = (*(*cb).aregs).fa[fresh0 as usize].d;
            return;
        }
        _ => {}
    }
    if (*cb).used_integer == NARGREG {
        let fresh1 = (*cb).used_stack;
        (*cb).used_stack = (*cb).used_stack.offset(1);
        value = *fresh1;
    } else {
        let fresh2 = (*cb).used_integer;
        (*cb).used_integer = (*cb).used_integer + 1;
        value = (*(*cb).aregs).a[fresh2 as usize];
    }
    match type_0 {
        FFI_TYPE_UINT8 => {
...
unsafe extern "C" fn marshal(
    ...
    if (*type_0).size > (2 as ::core::ffi::c_int * __SIZEOF_POINTER__) as size_t {
        ...
        (*cb).struct_stack = ((((*cb).struct_stack as *mut ::core::ffi::c_char)
            .offset((*type_0).size as isize) as size_t)
            .wrapping_sub(1 as size_t)
...
unsafe extern "C" fn ffi_call_int(
    ...
    // C2Rust lowers alloca to a Vec, but the upstream assembly temporarily
    // uses this allocation as the callee stack. Reserve bounded downward
    // stack space and align the argument region. The assembly separately
    // saves/restores the real stack pointer in the enlarged context frame.
    const CALLEE_STACK_HEADROOM: size_t = 1024 * 1024;
    alloca_allocations.push(::std::vec::from_elem(
        0,
        alloc_size
            .wrapping_add(CALLEE_STACK_HEADROOM)
            .wrapping_add(STKALIGN as size_t - 1) as usize,
    ));
    let heap_base = alloca_allocations.last_mut().unwrap().as_mut_ptr() as size_t;
    let mut alloc_base: size_t = (heap_base
        .wrapping_add(CALLEE_STACK_HEADROOM)
        .wrapping_sub(1)
    ...
    if return_by_ref == 0 && !rvalue.is_null() {
        if (*(*cif).rtype).type_0 as ::core::ffi::c_int >= FFI_TYPE_UINT8
            ...
            && (*(*cif).rtype).size < ::core::mem::size_of::<ffi_arg>() as usize
        {
...
pub unsafe extern "C" fn ffi_prep_closure_loc(
    ...
    if ffi_tramp_is_present(closure as *mut ::core::ffi::c_void) != 0 {
        let mut dest: Option<unsafe extern "C" fn() -> ()> =
            Some(ffi_closure_asm as unsafe extern "C" fn() -> ());
        ffi_tramp_set_parms(
        ...
        let mut tramp: *mut uint32_t = (&raw mut (*closure).c2rust_unnamed.tramp
            as *mut ::core::ffi::c_char)
            .offset(0 as ::core::ffi::c_int as isize)
            as *mut ::core::ffi::c_char as *mut uint32_t;
        let mut fn_0: uint64_t =
            ::core::mem::transmute::<Option<unsafe extern "C" fn() -> ()>, uintptr_t>(Some(
                ffi_closure_asm as unsafe extern "C" fn() -> (),
            )) as uint64_t;
        *tramp.offset(0 as ::core::ffi::c_int as isize) = 0x317 as uint32_t;
        *tramp.offset(1 as ::core::ffi::c_int as isize) = 0x1033383 as uint32_t;
        *tramp.offset(2 as ::core::ffi::c_int as isize) = 0x38067 as uint32_t;
        *tramp.offset(3 as ::core::ffi::c_int as isize) = 0x13 as uint32_t;
        *tramp.offset(4 as ::core::ffi::c_int as isize) = fn_0 as uint32_t;
        *tramp.offset(5 as ::core::ffi::c_int as isize) =
            (fn_0 >> 32 as ::core::ffi::c_int) as uint32_t;
        __riscv_flush_icache(
            codeloc,
            codeloc.offset(FFI_TRAMPOLINE_SIZE as isize),
