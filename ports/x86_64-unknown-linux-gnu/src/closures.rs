...
unsafe fn atomic_load_acquire_u64(ptr: *mut usize) -> usize {
    (*(ptr as *const ::core::sync::atomic::AtomicUsize))
        .load(::core::sync::atomic::Ordering::Acquire)
}
unsafe fn atomic_store_release_u64(ptr: *mut usize, value: usize) {
    (*(ptr as *const ::core::sync::atomic::AtomicUsize))
        .store(value, ::core::sync::atomic::Ordering::Release);
}
#[repr(C)]
        ...
        if bytes <= MAX_SMALL_REQUEST {
                ...
                != 0 as ::core::ffi::c_uint
            {
                ...
                mem = (p as *mut ::core::ffi::c_char).offset(TWO_SIZE_T_SIZES as usize as isize)
                    as *mut ::core::ffi::c_void;
                current_block = 5472325052213876983;
                ...
                != 0
            {
                ...
                if (*p).head & PINUSE_BIT == 0 {
                    ...
                    if (*p).head & INUSE_BITS == 0 as size_t {
                        ...
                        current_block = 389218827533125586;
                    } else {
                            ...
                            != 0
                        {
                            ...
                            current_block = 6803970968623740219;
                        }
                    }
                } else {
                    current_block = 10778260831612459202;
...
unsafe extern "C" fn dlmmap_locked(
    ...
    if execfd == -(1 as ::core::ffi::c_int) {
        open_temp_exec_file_opts_idx = 0 as ::core::ffi::c_int;
        current_block = 16568002056926884701;
    } else {
        current_block = 735147466149431745;
    }
    loop {
        match current_block {
            16568002056926884701 => {
