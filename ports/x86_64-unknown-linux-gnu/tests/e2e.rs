...
fn reports_libffi_version() {
    unsafe {
        assert_eq!(
            libffi_c2rust_x86_64_gnu::src::types::ffi_get_version_number(),
            30502
        );
    }
}
...
#[test]
fn long_double_descriptors_match_x86_64_gnu() {
    unsafe {
        let long_double = &raw const ffi_type_longdouble;
        assert_eq!((*long_double).size, 16);
        assert_eq!((*long_double).alignment, 16);
        let complex_float = &raw const ffi_type_complex_float;
        assert_eq!((*complex_float).size, 8);
        assert_eq!((*complex_float).alignment, 4);
        let complex_double = &raw const ffi_type_complex_double;
        assert_eq!((*complex_double).size, 16);
        assert_eq!((*complex_double).alignment, 8);
        let complex_long_double = &raw const ffi_type_complex_longdouble;
        assert_eq!((*complex_long_double).size, 32);
        assert_eq!((*complex_long_double).alignment, 16);
    }
}
...
// point aggregate also exercises small aggregate classification on x86-64.
#[derive(Clone, Copy)]
#[repr(C)]
struct ThreeFloats {
    f1: f32,
