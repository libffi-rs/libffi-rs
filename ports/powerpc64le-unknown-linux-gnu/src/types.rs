...
pub static mut ffi_type_longdouble: ffi_type = _ffi_type {
    // The configured ELFv2 PPC64LE ABI uses the IBM double-double long-double
    // format: 16-byte storage, 16-byte alignment, and FFI_TYPE_LONGDOUBLE.
    // No Rust numeric type is used here because this is an ABI descriptor.
    size: 16,
    alignment: 16,
    type_0: 4,
...
// These arrays are immutable after initialization and point only at the
// process-wide ffi_type descriptors, matching libffi's C initializers.
unsafe impl Sync for FfiElements {}

static FFI_ELEMENTS_COMPLEX_FLOAT: FfiElements = FfiElements([
    &raw const ffi_type_float as *mut ffi_type,
    ::core::ptr::null_mut(),
]);
static FFI_ELEMENTS_COMPLEX_DOUBLE: FfiElements = FfiElements([
    &raw const ffi_type_double as *mut ffi_type,
    ::core::ptr::null_mut(),
]);
static FFI_ELEMENTS_COMPLEX_LONGDOUBLE: FfiElements = FfiElements([
