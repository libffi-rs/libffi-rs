...
unsafe fn atomic_load_relaxed<T: Copy>(p: *const T) -> T {
    use core::sync::atomic::{AtomicI32, AtomicUsize, Ordering};
    if core::mem::size_of::<T>() == 4 {
        core::mem::transmute_copy(&(&*p.cast::<AtomicI32>()).load(Ordering::Relaxed))
    } else {
        core::mem::transmute_copy(&(&*p.cast::<AtomicUsize>()).load(Ordering::Relaxed))
    }
}
unsafe fn atomic_load_acquire<T: Copy>(p: *const T) -> T {
    use core::sync::atomic::{AtomicI32, AtomicUsize, Ordering};
    if core::mem::size_of::<T>() == 4 {
        core::mem::transmute_copy(&(&*p.cast::<AtomicI32>()).load(Ordering::Acquire))
    } else {
        core::mem::transmute_copy(&(&*p.cast::<AtomicUsize>()).load(Ordering::Acquire))
    }
}
unsafe fn atomic_xchg_acquire<T: Copy>(p: *mut T, v: T) -> T {
    use core::sync::atomic::{AtomicI32, AtomicUsize, Ordering};
    if core::mem::size_of::<T>() == 4 {
        let v = core::mem::transmute_copy::<T, i32>(&v);
        core::mem::transmute_copy(&(&*p.cast::<AtomicI32>()).swap(v, Ordering::Acquire))
    } else {
        let v = core::mem::transmute_copy::<T, usize>(&v);
        core::mem::transmute_copy(&(&*p.cast::<AtomicUsize>()).swap(v, Ordering::Acquire))
    }
}
unsafe fn atomic_store_release<T: Copy>(p: *mut T, v: T) {
    use core::sync::atomic::{AtomicI32, AtomicUsize, Ordering};
    if core::mem::size_of::<T>() == 4 {
        (&*p.cast::<AtomicI32>()).store(core::mem::transmute_copy::<T, i32>(&v), Ordering::Release)
    } else {
        (&*p.cast::<AtomicUsize>())
            .store(core::mem::transmute_copy::<T, usize>(&v), Ordering::Release)
    }
}
        ...
        if bytes <= MAX_SMALL_REQUEST {
                ...
                != 0 as ::core::ffi::c_uint
            {
                ...
                mem = (p as *mut ::core::ffi::c_char).offset(TWO_SIZE_T_SIZES as usize as isize)
                    as *mut ::core::ffi::c_void;
                current_block = 5238444436672145426;
...
unsafe extern "C" fn dlmmap_locked(
    ...
    if execfd == -(1 as ::core::ffi::c_int) {
        open_temp_exec_file_opts_idx = 0 as ::core::ffi::c_int;
        current_block = 14584359294019003606;
    } else {
        current_block = 735147466149431745;
    }
    loop {
        match current_block {
            14584359294019003606 => {
