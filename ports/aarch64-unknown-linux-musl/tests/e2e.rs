...
#[test]
fn target_abi_layout() {
    assert_eq!(FFI_DEFAULT_ABI, 1);
    assert_eq!(core::mem::size_of::<FfiCif>(), 32);
    unsafe { assert_eq!(ffi_get_closure_size(), 48) };
}
...
fn reports_libffi_version() {
    unsafe {
        assert_eq!(
            libffi_c2rust_aarch64_musl::src::types::ffi_get_version_number(),
            30502
        );
    }
}
...
#[test]
fn long_double_descriptors_match_aarch64_musl() {
    unsafe {
        ...
        assert_eq!((*complex).size, 32);
        assert_eq!((*complex).alignment, 16);
    }
}
...
// point aggregate exercises AArch64 HFA argument and return classification.
#[derive(Clone, Copy)]
#[repr(C)]
struct ThreeFloats {
    f1: f32,
