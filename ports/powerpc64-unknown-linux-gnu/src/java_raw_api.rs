    ...
    while i < (*cif).nargs {
        match (**tp).type_0 as ::core::ffi::c_int {
            FFI_TYPE_UINT8 | FFI_TYPE_SINT8 => {
                let fresh0 = raw;
                raw = raw.offset(1);
                *args = (fresh0 as *mut ::core::ffi::c_char)
            ...
            FFI_TYPE_UINT16 | FFI_TYPE_SINT16 => {
                let fresh1 = raw;
                raw = raw.offset(1);
                *args = (fresh1 as *mut ::core::ffi::c_char)
            ...
            FFI_TYPE_POINTER => {
                let fresh2 = raw;
                raw = raw.offset(1);
                *args = &raw mut (*fresh2).ptr as *mut ::core::ffi::c_void;
            }
            FFI_TYPE_COMPLEX => {
                abort();
            }
    ...
    while i < (*cif).nargs {
        match (**tp).type_0 as ::core::ffi::c_int {
            FFI_TYPE_UINT8 => {
                let fresh3 = raw;
                raw = raw.offset(1);
                *(fresh3 as *mut UINT32) = *(*args as *mut UINT8) as UINT32;
            }
            FFI_TYPE_SINT8 => {
                let fresh4 = raw;
                raw = raw.offset(1);
                *(fresh4 as *mut SINT32) = *(*args as *mut SINT8) as SINT32;
            }
            FFI_TYPE_UINT16 => {
                let fresh5 = raw;
                raw = raw.offset(1);
                *(fresh5 as *mut UINT32) = *(*args as *mut UINT16) as UINT32;
            }
            FFI_TYPE_SINT16 => {
                let fresh6 = raw;
                raw = raw.offset(1);
                *(fresh6 as *mut SINT32) = *(*args as *mut SINT16) as SINT32;
            }
            FFI_TYPE_UINT32 => {
                let fresh7 = raw;
                raw = raw.offset(1);
                *(fresh7 as *mut UINT32) = *(*args as *mut UINT32);
            }
            FFI_TYPE_SINT32 => {
                let fresh8 = raw;
                raw = raw.offset(1);
                *(fresh8 as *mut SINT32) = *(*args as *mut SINT32);
            }
            FFI_TYPE_FLOAT => {
                let fresh9 = raw;
                raw = raw.offset(1);
                (*fresh9).flt = *(*args as *mut FLOAT32) as ::core::ffi::c_float;
            }
            FFI_TYPE_UINT64 | FFI_TYPE_SINT64 | FFI_TYPE_DOUBLE => {
...
unsafe extern "C" fn ffi_java_rvalue_to_raw(
    ...
    match (*(*cif).rtype).type_0 as ::core::ffi::c_int {
        FFI_TYPE_UINT8 | FFI_TYPE_UINT16 | FFI_TYPE_UINT32 => {
            *(rvalue as *mut UINT64) <<= 32 as ::core::ffi::c_int;
        }
        FFI_TYPE_SINT8 | FFI_TYPE_SINT16 | FFI_TYPE_SINT32 | FFI_TYPE_INT => {
            *(rvalue as *mut SINT64) <<= 32 as ::core::ffi::c_int;
        }
        FFI_TYPE_COMPLEX => {
            abort();
        }
