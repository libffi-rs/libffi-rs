        ...
        match (**tp).type_0 as ::core::ffi::c_int {
            FFI_TYPE_UINT8 | FFI_TYPE_SINT8 => {
                ...
                *args = (fresh0 as *mut ::core::ffi::c_char)
                    .offset(FFI_SIZEOF_ARG as isize)
            ...
            FFI_TYPE_UINT32 | FFI_TYPE_SINT32 => {
                let fresh2 = raw;
                raw = raw.offset(1);
                *args = (fresh2 as *mut ::core::ffi::c_char)
                    .offset(FFI_SIZEOF_ARG as isize)
            ...
            FFI_TYPE_POINTER => {
                let fresh5 = raw;
                raw = raw.offset(1);
                *args = &raw mut (*fresh5).ptr as *mut ::core::ffi::c_void;
            }
            _ => {
    ...
    while i < (*cif).nargs {
        match (**tp).type_0 as ::core::ffi::c_int {
            FFI_TYPE_UINT8 => {
                let fresh6 = raw;
                raw = raw.offset(1);
                (*fresh6).uint = *(*args as *mut UINT8) as ffi_arg;
            }
            FFI_TYPE_SINT8 => {
                let fresh7 = raw;
                raw = raw.offset(1);
                (*fresh7).sint = *(*args as *mut SINT8) as ffi_sarg;
            }
            FFI_TYPE_UINT16 => {
                let fresh8 = raw;
                raw = raw.offset(1);
                (*fresh8).uint = *(*args as *mut UINT16) as ffi_arg;
            }
            FFI_TYPE_SINT16 => {
                let fresh9 = raw;
                raw = raw.offset(1);
                (*fresh9).sint = *(*args as *mut SINT16) as ffi_sarg;
            }
            FFI_TYPE_UINT32 => {
                let fresh10 = raw;
                raw = raw.offset(1);
                (*fresh10).uint = *(*args as *mut UINT32) as ffi_arg;
            }
            FFI_TYPE_SINT32 => {
                let fresh11 = raw;
                raw = raw.offset(1);
                (*fresh11).sint = *(*args as *mut SINT32) as ffi_sarg;
            }
            FFI_TYPE_STRUCT => {
                let fresh12 = raw;
                raw = raw.offset(1);
            ...
            FFI_TYPE_POINTER => {
                let fresh14 = raw;
                raw = raw.offset(1);
                (*fresh14).ptr = **(args as *mut *mut *mut ::core::ffi::c_void);
            }
            _ => {
                memcpy(
...
unsafe extern "C" fn ffi_translate_args(
    mut cif: *mut ffi_cif,
    mut rvalue: *mut ::core::ffi::c_void,
    ...
    let mut alloca_allocations: Vec<Vec<u8>> = Vec::new();
    alloca_allocations.push(::std::vec::from_elem(0, ffi_raw_size(cif) as usize));
    let mut raw: *mut ffi_raw = alloca_allocations.last_mut().unwrap().as_mut_ptr() as *mut ffi_raw;
