...
pub struct call_frame {
    ...
    pub saved_sp: *mut ::core::ffi::c_void,
}
...
unsafe extern "C" fn ffi_call_int(
    ...
    // C2Rust lowers C alloca to Vec, but ARM sysv.S installs this buffer as
    // SP while invoking the callee. Reserve downward-growing stack space so
    // the callee cannot overwrite allocator metadata before the Vec buffer.
    const CALL_STACK_RESERVE: usize = 1024 * 1024;
    alloca_allocations.push(::std::vec::from_elem(
        0,
        CALL_STACK_RESERVE
            .wrapping_add(vfp_size as usize)
            .wrapping_add(bytes as usize)
            .wrapping_add(::core::mem::size_of::<call_frame>() as usize)
            .wrapping_add(rsize as usize) as usize,
    ));
    stack = alloca_allocations
        .last_mut()
        .unwrap()
        .as_mut_ptr()
        .add(CALL_STACK_RESERVE) as *mut ::core::ffi::c_char;
    ...
    if rsize != 0 {
        new_rvalue = frame.offset(1 as ::core::ffi::c_int as isize) as *mut ::core::ffi::c_void
    ...
    if !rvalue.is_null() && rvalue != new_rvalue as *mut ::core::ffi::c_void {
        memcpy(
            rvalue,
            new_rvalue as *const ::core::ffi::c_void,
            (*rtype).size,
        );
    }
}
...
pub unsafe extern "C" fn ffi_prep_closure_loc(
    ...
    let closure_func: unsafe extern "C" fn() = if (*cif).abi == FFI_VFP {
        if (*cif).vfp_used != 0 {
            ffi_closure_VFP
        } else {
            ffi_closure_SYSV
    ...
    let trampoline = (&raw mut (*closure).c2rust_unnamed.tramp).cast::<u8>();
    memcpy(trampoline.cast(), (&raw const ffi_arm_trampoline).cast(), 8);
    // Linux ARM private cacheflush syscall; avoids a libgcc dependency.
    const ARM_NR_CACHEFLUSH: ::core::ffi::c_long = 0x0f0002;
    syscall(ARM_NR_CACHEFLUSH, trampoline, trampoline.add(8), 0usize);
    syscall(
        ARM_NR_CACHEFLUSH,
        codeloc,
        codeloc.cast::<u8>().add(8),
        0usize,
    );
    trampoline
        .add(8)
