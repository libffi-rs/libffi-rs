    ...
    while i >= 0 as ::core::ffi::c_int {
        match (**at).type_0 as ::core::ffi::c_int {
            FFI_TYPE_UINT64 | FFI_TYPE_SINT64 | FFI_TYPE_DOUBLE => {
                result = result
                    .wrapping_add(
                        (2 as ::core::ffi::c_int * FFI_SIZEOF_JAVA_RAW) as size_t,
                    );
            }
            FFI_TYPE_STRUCT => {
                abort();
...
pub unsafe extern "C" fn ffi_java_ptrarray_to_raw(
    ...
    while i < (*cif).nargs {
        match (**tp).type_0 as ::core::ffi::c_int {
            ...
            _ => {
                memcpy(
                    &raw mut (*raw).data as *mut ::core::ffi::c_char
...
unsafe extern "C" fn ffi_java_translate_args(
    ...
    let mut raw: *mut ffi_java_raw = alloca_allocations.last_mut().unwrap().as_mut_ptr()
        as *mut ffi_java_raw;
    let mut cl: *mut ffi_raw_closure = user_data as *mut ffi_raw_closure;
    ffi_java_ptrarray_to_raw(cif, avalue, raw);
    Some((*cl).fun.expect("non-null function pointer"))
        .expect(
            "non-null function pointer",
        )(cif, rvalue, raw as *mut ffi_raw, (*cl).user_data);
...
pub unsafe extern "C" fn ffi_prep_java_raw_closure(
    ...
    return ffi_prep_java_raw_closure_loc(
        cl,
        cif,
        fun,
        user_data,
        cl as *mut ::core::ffi::c_void,
    );
}
