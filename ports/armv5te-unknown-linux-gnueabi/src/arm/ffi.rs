extern "C" {
    ...
    // GCC's ARM libgcc cache maintenance routine (the C source uses the builtin).
...
pub unsafe extern "C" fn ffi_prep_closure_loc(
    ...
    // FFI_EXEC_STATIC_TRAMP was selected by configure.
    if ffi_tramp_is_present(closure.cast()) != 0 {
        ...
        ffi_tramp_set_parms(
            ...
            closure_func as *mut _,
            closure.cast(),
        );
    } else {
        let tramp = ::core::ptr::addr_of_mut!((*closure).c2rust_unnamed.tramp).cast::<u8>();
        memcpy(
            tramp.cast(),
            ::core::ptr::addr_of!(ffi_arm_trampoline).cast(),
            8,
        );
        __clear_cache(tramp.cast(), tramp.add(8).cast());
        let code = codeloc.cast::<u8>();
        __clear_cache(code.cast(), code.add(8).cast());
        tramp
            .add(8)
            .cast::<*mut ::core::ffi::c_void>()
            .write(closure_func as *mut _);
    }
    (*closure).cif = cif;
    (*closure).fun = fun;
