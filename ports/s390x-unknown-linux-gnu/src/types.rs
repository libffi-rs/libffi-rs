...
pub static mut ffi_type_longdouble: ffi_type = _ffi_type {
    // s390x GNU/Linux uses IEEE binary128: 16 bytes with 8-byte alignment.
    // Rust has no stable C-compatible long-double scalar, but ffi_type only
    // describes its storage; all target call paths pass it by reference.
    size: 16,
    alignment: 8,
...
// The arrays are immutable after static initialization and their pointers refer
// only to process-wide ffi_type descriptors.
...
// C2Rust 0.22.1 reports unsupported C complex static initializers, so these
// descriptors are spelled out using the s390x values measured from GCC.
#[no_mangle]
