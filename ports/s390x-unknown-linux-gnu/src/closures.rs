...
trait C2RustAtomic: Copy {
    unsafe fn load(ptr: *const Self, ordering: Ordering) -> Self;
    unsafe fn swap(ptr: *mut Self, value: Self, ordering: Ordering) -> Self;
    unsafe fn store(ptr: *mut Self, value: Self, ordering: Ordering);
}

impl C2RustAtomic for i32 {
    unsafe fn load(ptr: *const Self, ordering: Ordering) -> Self {
        AtomicI32::from_ptr(ptr.cast_mut()).load(ordering)
    }
    unsafe fn swap(ptr: *mut Self, value: Self, ordering: Ordering) -> Self {
        AtomicI32::from_ptr(ptr).swap(value, ordering)
    }
    unsafe fn store(ptr: *mut Self, value: Self, ordering: Ordering) {
        AtomicI32::from_ptr(ptr).store(value, ordering);
    }
}

impl C2RustAtomic for usize {
    unsafe fn load(ptr: *const Self, ordering: Ordering) -> Self {
        AtomicUsize::from_ptr(ptr.cast_mut()).load(ordering)
    }
    unsafe fn swap(ptr: *mut Self, value: Self, ordering: Ordering) -> Self {
        AtomicUsize::from_ptr(ptr).swap(value, ordering)
    }
    unsafe fn store(ptr: *mut Self, value: Self, ordering: Ordering) {
        AtomicUsize::from_ptr(ptr).store(value, ordering);
    }
}

unsafe fn atomic_load_relaxed<T: C2RustAtomic>(ptr: *const T) -> T {
    T::load(ptr, Ordering::Relaxed)
}
unsafe fn atomic_load_acquire<T: C2RustAtomic>(ptr: *const T) -> T {
    T::load(ptr, Ordering::Acquire)
}
unsafe fn atomic_xchg_acquire<T: C2RustAtomic>(ptr: *mut T, value: T) -> T {
    T::swap(ptr, value, Ordering::Acquire)
}
unsafe fn atomic_store_release<T: C2RustAtomic>(ptr: *mut T, value: T) {
    T::store(ptr, value, Ordering::Release);
}
...
unsafe extern "C" fn selinux_enabled_check() -> ::core::ffi::c_int {
    ...
    if statfs(
        ...
        && sfs.f_type == 0xf97cff8c as ::core::ffi::c_uint
    {
                ...
                != 0
            {
                ...
                if (*p).head & PINUSE_BIT == 0 {
                    ...
                    if (*p).head & INUSE_BITS == 0 as size_t {
                        ...
                        current_block = 934771290496120177;
                    } else {
                            ...
                            != 0
                        {
                            ...
                            current_block = 3343522689139624673;
                        }
                    }
                } else {
                    current_block = 10778260831612459202;
...
unsafe extern "C" fn dlmmap_locked(
    ...
    if execfd == -(1 as ::core::ffi::c_int) {
        open_temp_exec_file_opts_idx = 0 as ::core::ffi::c_int;
        current_block = 11822221851119215373;
    } else {
        current_block = 735147466149431745;
    }
    loop {
        match current_block {
            11822221851119215373 => {
