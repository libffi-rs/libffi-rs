...
unsafe extern "C" fn marshal_atom(
    ...
    match type_0 {
        ...
        FFI_TYPE_DOUBLE => {
            ...
            (*cb).used_float = (*cb).used_float + 1;
            let fresh9 = &mut (*(*cb).aregs).fa[fresh8 as usize].d;
            let fresh10;
            let fresh11 = *(data as *mut ::core::ffi::c_double);
            asm!(
                "\n", inlateout(freg) c2rust_asm_casts::AsmCast::cast_in(fresh9, fresh11)
                => fresh10, options(preserves_flags, pure, readonly)
            );
...
unsafe extern "C" fn unmarshal_atom(
    ...
    match type_0 {
        ...
        FFI_TYPE_DOUBLE => {
            let fresh0 = &mut *(data as *mut ::core::ffi::c_double);
            let fresh1;
            let fresh2 = (*cb).used_float;
            (*cb).used_float = (*cb).used_float + 1;
            let fresh3 = (*(*cb).aregs).fa[fresh2 as usize].d;
            asm!(
                "\n", inlateout(freg) c2rust_asm_casts::AsmCast::cast_in(fresh0, fresh3)
                => fresh1, options(preserves_flags, pure, readonly)
            );
            c2rust_asm_casts::AsmCast::cast_out(fresh0, fresh3, fresh1);
            return;
        }
        _ => {}
    }
...
unsafe extern "C" fn ffi_call_int(
    ...
    if ::core::mem::align_of::<max_align_t>() >= STKALIGN as usize {
        ...
        alloca_allocations.push(::std::vec::from_elem(
            0,
            (alloc_size as ::core::ffi::c_ulong)
                .wrapping_add(16 as ::core::ffi::c_ulong)
                .wrapping_sub(1 as ::core::ffi::c_ulong) as usize,
        ));
        alloc_base = ((alloca_allocations.last_mut().unwrap().as_mut_ptr() as ::core::ffi::c_ulong)
            .wrapping_sub(1 as ::core::ffi::c_ulong)
            | (16 as ::core::ffi::c_int - 1 as ::core::ffi::c_int) as ::core::ffi::c_ulong)
            .wrapping_add(1 as ::core::ffi::c_ulong) as size_t;
    }
...
pub unsafe extern "C" fn ffi_closure_inner(
    ...
    if return_by_ref != 0 {
        ...
        rvalue = alloca_allocations.last_mut().unwrap().as_mut_ptr();
    }
    i = 0 as ::core::ffi::c_int;
    while (i as ::core::ffi::c_uint) < (*cif).nargs {
